...
def analyze_repo_topology(
    ...
    split_preview = bool((phase_cmds or {}).get("studio_split_preview_mode"))
    if frontish and localhost_refs > 0 and not split_preview:
        issues.append(
            {
    ...
    if frontish and vite_proxy_hint and not split_preview:
        issues.append(
            {
    ...
    return {
        ...
        "lifecycle_category_hint": ("topology_mismatch" if issues else "post_start_application_state"),
